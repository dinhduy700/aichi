...
class XlsPickingGurisuto extends XlsExportMstMultiRowBlock
{
    public function addDataToExcel($objPHPExcel, $config, $data)
    {
        ...
        $dataGroupBy = $data->groupBy(function ($item, $key) use($config) {
            $colsGroupBy = [
                $item->bumon_cd,
                $item->kisan_dt,
            ];

            if (data_get($config, 'is_syaban')) {
                $colsGroupBy[] = $item->syaban;
            }

            if (data_get($config, 'is_hachaku')) {
                $colsGroupBy[] = $item->hachaku_cd;
            }
        ...
        foreach ($dataGroupBy as $key => $items) {
            $items          = $items->sortBy('todokesaki_nm');
            ...
            foreach ($chunks as $key => $chunk) {
                ...
                foreach ($chunk as $k => $item) {
                    if ( isset($prev[data_get($item, 'todokesaki_nm')])) {
                        if (in_array(data_get($item, 'ninusi1_nm'), $prev[data_get($item, 'todokesaki_nm')])) {
                            if (data_get($item, 'ninusi1_nm') == $prevNinusiNm) {
                                data_set($item, 'ninusi1_nm', null);
                            } else {
                                $prevNinusiNm = data_get($item, 'ninusi1_nm');
                            }
                        } else {
                            $prev[data_get($item, 'todokesaki_nm')][] = data_get($item, 'ninusi1_nm');
                            $prevNinusiNm = data_get($item, 'ninusi1_nm');
                        }

                        data_set($item, 'todokesaki_nm', null);
                    } else {
                        $prev[data_get($item, 'todokesaki_nm')][] = data_get($item, 'ninusi1_nm');
                        $prevNinusiNm = data_get($item, 'ninusi1_nm');
                    }
