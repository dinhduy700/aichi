...
use App\Helpers\Excel\XlsSeikyuMikakutei;
use App\Helpers\Formatter;
use App\Http\Controllers\Controller;
use App\Http\Repositories\Seikyu\SeikyuMikakuteiRepository;
use App\Http\Requests\Seikyu\SeikyuListRequest;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\File;

class SeikyuMikakuteiController extends Controller
{
    protected $repository;
    public function __construct(SeikyuMikakuteiRepository $repository)
    {
    ...
    public function index(Request $request)
    {
        ...
        return view('seikyu.mikakutei.index', ['setting' => $setting, 'request' => $request]);
    }
    ...
    public function filterForm(Request $request)
    {
        ...
        return view('seikyu.mikakutei.exp-filter-form', compact('optionOpts', 'initValues', 'selectedItems'));
    }
    ...
    public function excel(Request $request)
    {
        ...
        $fileNm = date('YmdHis') . '_t_seikyu_mikakutei.xlsx';
        return response()->download(
            $this->exportExcel($request, $outDir, $fileNm),
            getPageTitle('seikyu.mikakutei.filterForm') . '_' . date('YmdHis') . '.xlsx'
    ...
    public function pdf(Request $request)
    {
        ...
        $fileName = getPageTitle('seikyu.mikakutei.filterForm') . '_' . date('YmdHis') . '.pdf';
    ...
    {
        $exporter = new XlsSeikyuMikakutei();
        $qb = $this->repository->applyRequestToBuilder($request);
        $data = $qb->get();

        $config = require(app_path('Helpers/Excel/config/t_seikyu_mikakutei.php'));
        ...
        $exporter->export(
            app_path('Helpers/Excel/template/t_seikyu_mikakutei.xlsx'),//template
            $config,
            $data,
            $savePath
        );
    ...
    public function  csv(Request $request)
    {
        ...
        $mapping = require(app_path('Helpers/Csv/config/t_seikyu_mikakutei.php'));
        ...
        return response()->download(
            $savePath,
            getPageTitle('seikyu.mikakutei.filterForm') . '_' . date('YmdHis') . '.csv',
